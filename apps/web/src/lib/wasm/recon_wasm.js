 ...
 */
export function input_begin(expected_len) {
    const ret = wasm.input_begin(expected_len);
    if (ret[1]) {
 ...
 */
export function input_push(chunk) {
    wasm.input_push(chunk);
}
 ...
 * @param {number} row_interval
 * @returns {any}
 */
export function parse_input(on_progress, row_interval) {
    const ret = wasm.parse_input(on_progress, row_interval);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function parse_recon_bytes(file_bytes) {
    const ptr0 = passArray8ToWasm0(file_bytes, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.parse_recon_bytes(ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('recon_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
